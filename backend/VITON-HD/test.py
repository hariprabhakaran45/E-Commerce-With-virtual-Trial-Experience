...
def get_opt():
    ...
    parser.add_argument('--save_dir', type=str, default='./VITON-HD./results/')
    ...
    with torch.no_grad():
        for i, inputs in enumerate(test_loader.data_loader):
            ...
            for img_name, c_name in zip(img_names, c_names):
                result_image_name = '{}_{}'.format(img_name.split('_')[0], c_name.split('_')[0] + '_' + c_name.split('_')[1])
                unpaired_names.append(result_image_name)

            # Ensure the directory for saving results exists
            save_folder = os.path.join(opt.save_dir, opt.name)
            if not os.path.exists(save_folder):
                os.makedirs(save_folder)

            save_images(output, unpaired_names, save_folder)
