    ...
    def __init__(self, opt):
        ...
        with open(osp.join(opt.dataset_dir, opt.dataset_list), 'r') as f:
            for line in f.readlines():
                ...
                if len(parts) == 2:
                    img_name, c_name = parts
                    img_names.append(img_name)
                    c_names.append(c_name)
                else:
                    print(f"Warning: Skipping invalid line (should contain exactly two filenames): {line}")
...
class VITONDataLoader:
    def __init__(self, opt, dataset):
        ...
        self.data_loader = data.DataLoader(
                ...
                num_workers=opt.workers, pin_memory=False, drop_last=True, sampler=train_sampler
        )
